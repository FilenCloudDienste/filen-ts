...
- **patch-package**: applied via `scripts/postinstall.sh`. Patches in `patches/` — notable: xcode 3.0.1 (file-provider `addResourceFile` null guard), expo-video 57 (Android PiP-exit reparent crash), react-native-screens 4.25.2 (Android/Fabric: `SurfaceMountingManager.updateLayout` resurrects header subviews hidden by the open search bar — e.g. rotation on cutout devices painted the overflow menu top-left; fixed by pinning `isHiddenBySearchBar`, upstreamable), @blazejkustra/react-native-alert 1.0.0 (Android: non-login prompt dialogs excluded from autofill — their password-variation input types made Google Password Manager pop on plain text prompts, upstreamable), expo-screen-orientation 57.0.0 (iOS: missing dispatch barriers on controller register/unregister — plain `sync` writes on the registry's concurrent queue race the barrier-less reader loop and corrupt the controllers array, crashing at AppContext teardown; upstreamable), expo-sharing 57.0.6 (Android: a stale `pendingPromise` — the chooser's activity result was never delivered, seen on newer Android sharesheets — permanently rejected every later share with ERR_SHARING_IN_PROGRESS; settle-and-clear at `shareAsync` entry instead of throwing, upstreamable; paired with the JS settle watchdog in `src/lib/share.ts`), plus expo-media-library and other version-bumped patches.
