...
Encrypted cloud storage mobile app — Expo 57 / React Native 0.86 (bridgeless/new-arch) / React 19 / Hermes.
All server communication, encryption, and auth handled by Rust SDK (`@filen/sdk-rs@0.4.42`, exact pin).
...
| `drivePreview/` | Pinch-to-dismiss gallery + per-type renderers (image/rawImage/video/audio/pdf/docx/text). `rawImage` (RAW camera files) renders the SDK-extracted JPEG via `previewRawImage` → `useRawPreviewQuery` inside `PreviewSlot`; `unavailableOfflineNotice` / `previewLoadFailedNotice` are the shared offline / load-failed pages (the latter also renders `previewText`'s error state).                                                                                          |
...
| `pdfPreview/`   | pdf.js viewer as an Expo DOM component (both platforms). Own page virtualization, text + annotation layers, bounded range reader, link funnel, password-widget hardening. Fonts and wasm decoders are inlined by `scripts/generatePdfjsAssets.ts` — rerun after every `pdfjs-dist` bump.                                                                                                                                                                                    |
| `docxPreview/`  | WebView-based `.docx` renderer.                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `shell/`        | App-shell background/overlay components (subscribe to one concern, render no rows). `socket.tsx` keeps the WebSocket connection lifecycle + a dispatcher delegating to per-feature `socketHandlers.ts`. Shell currently holds only `socket.tsx`; http, biometric, privacyScreen, offlineBanner, accountReminders and pathname live directly under `src/components/`. (foregroundService moved to features/transfers; incomingShareHandler moved to features/incomingShare.) |
...
| `thumbnails.ts`           | Thumbnail generation. Eligibility (`canGenerate`) = `getPreviewType` ∈ image/rawImage AND the SDK's own `canMakeThumbnail` flag, or video (displayability + the SDK's per-file gate; svg excluded; no format list for SDK support lives in the app). Images/RAW: local bytes (offline copy / file-cache hit, manipulator-decodable) → manipulator (`thumbnailsImage.ts`); otherwise the SDK (`thumbnailsSdk.ts`: `makeThumbnailInMemory` on `driveItemToAnyFile(item)`, 256×512 contain, ranged reads, lossless WebP, written `.webp.tmp` → `.webp`). `Unsupported`/`OverBudget`/`Corrupt` → one SESSION Set `unavailable` (no marker files), cleared on the online flip and by `clear()`; `generate()` resolves `null` for it, `isUnavailable(uuid)` is the sync read the row consults before mounting its generator. Offline with no local bytes → `OfflineAbortError` (uncounted). Video: expo-video-thumbnails via the HTTP provider behind `Semaphore(MAX_CONCURRENT)` (the SDK path is not). Post-upload local path (`generateFromLocalFile`) unchanged. Max 3 transient failures per item (aborts by `signal.aborted` / `name === "AbortError"` never count). Availability is a disk-derived Set (`hasThumbnail`), seeded once at boot by `restore()` from `*.webp`. |
| `thumbnailsSdk.ts`        | The SDK branch: `generateImageViaSdk({ file: AnyFile, uuid, outputPath, signal }) → "written" \| "settled"`, `THUMBNAIL_MAX_WIDTH`/`THUMBNAIL_MAX_HEIGHT`. Authed client only.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `rawPreviewCache.ts`      | Class singleton for the JPEG the SDK extracts from a RAW container (`writeEmbeddedPreviewToPath` into filen-tmp, then moved to `rawPreviews/v1/<uuid>.jpg`; uuid is the whole identity, `lastModified` is cachedAt). `has(item)` (sync), `get({ item, signal }) → { kind: "uri" } \| { kind: "noPreview" }` (nothing stored for noPreview), 24 h TTL + `RAW_PREVIEW_CACHE_MAX_SIZE_BYTES` (128 MiB) gc (debounced + on background), ClearBarrier `clear()`, `size()`. Wired into `useCacheSizes`, the advanced "clear preview cache" action and the logout wipe.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| `utils.ts`                | Shared helpers (`sanitizeFileName`, `contactDisplayName`, …). Path normalization (SDK/Expo/BlobUtil) lives in `paths.ts`; abort/pause plumbing (`wrapAbortSignalForSdk`, `toSignalOpts`, `PauseSignal`, composite signals) in `signals.ts`; preview classification in `previewType.ts`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `previewType.ts`          | `getPreviewType(name)` — the ONE displayability classifier (`image \| svg \| rawImage \| video \| audio \| pdf \| text \| code \| docx \| unknown`) behind gallery admission, photos grid, icons and thumbnail eligibility; `isImagePreviewType` = image/svg/rawImage. `SDK_RAW_PREVIEW_EXTENSIONS` (the ten RAW families, exported from here, NOT constants.ts): previewed via the SDK-extracted JPEG, never an expo-image input. Literal preview-type checks stay at chats inline, incomingShare, and the save-to-photos menu action; the save-to-photos bulk selector stays extension-based; RAW is excluded from save-to-photos either way, product decision.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| `logger.ts`               | Async, non-blocking on-disk diagnostic logger (class singleton). Hot path = level-gate + one cheap push (plus a synchronous `freezeForLog` SDK-error snapshot, since a uniffi handle may be freed before the deferred flush); JSON encoding/file append/rotation happen batched off-path at flush. warn/error (+ uncaught errors/rejections) persist to rotating NDJSON; info/debug live only in a bounded in-memory breadcrumb ring, dragged to disk as context in front of a persisted error. ~10MB cap. Prod gate (warn/error only) armed in the constructor. `purge()` on logout (terminal disable + wipe). `readEntries()` feeds the in-app viewer. A log call MUST NEVER throw. There is NO redaction layer: the SDK emits no secret material into log payloads, and by product decision logs KEEP decrypted names/paths/queries (what makes bugs findable; user warned at export).                                                                                                                                                                                                                                                                                                                                                                                   |
...
`src/lib/polyfills/` — loaded in order from `src/global.ts`: DOMException → buffer → crypto → console. `console.ts` is a TEE, not a plain replacement: every leveled `console.*` is forwarded into the diagnostic logger (`logger.captureConsole`), then in dev only forwarded to the native console (prod native stays silent but the output is captured to disk). warn/error args are run through SDK-error unwrapping first.
...
Default: refetchOnMount/Reconnect/Focus: "always", staleTime: 0, gcTime: 365 days, networkMode: "offlineFirst".
...
- `gallery` + `galleryItem` + `header` — pinch-to-dismiss image/rawImage/video/audio gallery
- `previewAudio` / `previewVideo` / `previewImage` / `previewRawImage` / `previewPdf` / `previewDocx` / `previewText` — per-type renderers (`previewRawImage`: `uri` → `previewImage`, `noPreview` → `ListEmpty` with `no_preview`, `offline` → `unavailableOfflineNotice`, error → `previewLoadFailedNotice`)
...
- **patch-package**: applied via `scripts/postinstall.sh`. Patches in `patches/` — notable: xcode 3.0.1 (file-provider `addResourceFile` null guard), expo-video 57 (Android PiP-exit reparent crash), @blazejkustra/react-native-alert 1.1.0 (Android: non-login prompt dialogs excluded from autofill — their password-variation input types made Google Password Manager pop on plain text prompts; the same variation also disabled IME composition, so dropping it fixed CJK/transliteration entry, and the non-secure branch now carries `CAP_SENTENCES` — Android capitalizes only when the field asks, so without it no prompt could ever produce a capital first letter, unlike iOS's UITextField default (#108). Upstreamable), @react-native-menu/menu 2.0.0 (Android: MenuView installs a TouchDelegate on its parent covering its hit rect + hitSlop and never uninstalls it — after the view unmounts, taps in the old rect are forwarded to the DETACHED view, whose gesture detector still shows the popup anchored at the window origin (a top-left menu; hit via the note header's saving-spinner swap). Detach now clears the parent delegate and prepareMenu bails on a detached anchor; upstreamable), expo-sharing 57.0.18 (Android: a stale `pendingPromise` — the chooser's activity result was never delivered, seen on newer Android sharesheets — permanently rejected every later share with ERR_SHARING_IN_PROGRESS; settle-and-clear at `shareAsync` entry instead of throwing, upstreamable; paired with the JS settle watchdog in `src/lib/share.ts`), expo-image-manipulator 57 (Android: `isActive` guard against double continuation resume from re-fired Glide callbacks, upstreamable), @shopify/flash-list 2.3.2 (`applyInitialScrollAdjustment` recomputed only `[0..initialScrollIndex]`, leaving the tail chained off the 200px default estimate — `layouts` stops being sorted by x at the seam and the binary search in `getVisibleLayouts`, which requires sortedness, mounts the wrong cells under a correct offset or returns EMPTY (flipping `isFirstLayoutComplete` vacuously, since `[].every()` is true). The `_recomputeLayouts` tail-resync cannot repair it on a HORIZONTAL list: it is gated on `y`, which is always 0 there. Recompute the whole table instead. Symptom: the drive-preview pager opened ~1.4x past the tapped item. Upstream Shopify/flash-list#2307 is open — DROP THIS PATCH once it ships), expo-background-task 57.0.16 (iOS: `handleTasksExpiredNotification` guarded on a `userInfo["url"]` that its only poster — the BGTask expirationHandler in BackgroundTaskAppDelegate — never sets, so `onTasksExpired` was never emitted and `addExpirationListener` has never fired in ANY released version; the binding was unused after the guard. Without it a BGProcessingTask expiry reaches JS nowhere and the app's own run-budget timer is the sole bound, so an expiry cuts transfers mid-flight and loses the run breadcrumb. Still broken on expo `main`, unreported upstream; upstreamable), plus expo-media-library and other version-bumped patches. (The former react-native-screens `isHiddenBySearchBar` patch was merged upstream and retired at 4.26.x.)
...
- **Diagnostic logging** — `src/lib/logger.ts` is an async, never-throwing on-disk NDJSON logger wired at `src/global.ts` (console tee + `installGlobalErrorHandlers()`) and torn down at logout (`logger.purge()` in `auth.ts`). Prod = warn/error only (debug/info are in-memory breadcrumbs), armed in the logger constructor; babel strips debug/info console call sites. Logs KEEP decrypted names/paths (what makes bugs findable; user warned at export) — there is no redaction layer, the SDK emits no secret material into log payloads. Surfaced via the `logViewer` route + `exportLogs()` in advanced settings; WebView console is proxied back via `hooks/useDomEvents/domConsoleProxy.ts`. From app code use `logger.warn/error(tag, msg, data)`; `console.*` is also captured.
