...
| `previewType.ts`          | `getPreviewType(name)` — the ONE displayability classifier (`image \| svg \| rawImage \| video \| audio \| pdf \| text \| code \| docx \| unknown`) behind gallery admission, photos grid, icons and thumbnail eligibility; `isImagePreviewType` = image/svg/rawImage. `SDK_RAW_PREVIEW_EXTENSIONS` (the ten RAW families, exported from here, NOT constants.ts): previewed via the SDK-extracted JPEG, never an expo-image input. Literal `"image"` stays at chats inline and incomingShare; save-to-photos (menu action + bulk selector) stays extension-based — RAW excluded, product decision.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
