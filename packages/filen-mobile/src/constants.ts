...
// Deliberately excluded: RAW camera files (.dng included) — they are classified "rawImage"
// (SDK_RAW_PREVIEW_EXTENSIONS in previewType.ts) and served on both platforms through the JPEG the
// SDK extracts from the container, so expo-image never sees RAW bytes whatever ImageIO could decode
// on iOS. They DO appear in the photos tab, admitted on that classification rather than this set.
// Also excluded: .psd/.heics on iOS (flattened PSD preview / HEIF sequences), .cur (zero prevalence).
// HEIC on an Android device without an HEVC decoder is the one known hole in "thumbnail ⇒
// previewable" (the SDK thumbnails it via libheif; expo-image cannot decode it there) —
// pre-existing preview limitation, rare on API 31+, accepted.
