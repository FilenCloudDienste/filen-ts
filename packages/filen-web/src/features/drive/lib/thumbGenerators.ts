...
import { transformImageThumb } from "@/features/drive/lib/imageThumb"
...
// Plain raster images (jpg/png/gif/webp/bmp/avif — whatever the browser decodes): pull the whole file
// through the same buffered byte path heic/pdf use, then decode + downscale + encode in the image
// worker (transformImageThumb). Replaces the former SDK makeThumbnailInMemory route — a full JS decode
// owned by the browser, which lets more formats through than the wasm decoder ever could. A download
// OR a decode failure (an unsupported or corrupt image) resolves null, which the service counts toward
// its 3-strike blacklist — an undecodable file fails permanently rather than retrying forever.
export const generateImageThumb: ThumbGenerator = async item => {
	let bytes: Uint8Array

	try {
		bytes = await downloadWholeFile(item)
	} catch (e) {
		log.warn("thumb-generators", "generateImageThumb: download failed", item.data.uuid, e)

		return null
	}

	try {
		return await transformImageThumb(bytes, THUMB_MAX_DIM)
	} catch (e) {
		log.warn("thumb-generators", "generateImageThumb: decode failed", item.data.uuid, e)
...
// module at least once, even just for this side effect, before requesting one; until then
// getThumbnailUrl's own unregistered-category path (a clean null, never a throw) is all any item
// resolves to.
registerThumbGenerator("image", generateImageThumb)
