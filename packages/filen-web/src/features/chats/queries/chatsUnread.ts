...
// Global unread-message count across every conversation — the icon-rail Chats badge (D4: in-app
// signals only, no favicon/title/OS notifications this campaign). `bigint` is fine in query DATA
...
// sdk.worker.ts's Chats section comment). That selector is a later (UI) wave's concern; this
...
// Confirm-then-patch write path (e.g. after markChatRead resolves, a later wave nudges this count
// down without waiting for a full refetch). No cancel-before-patch guard here, unlike
