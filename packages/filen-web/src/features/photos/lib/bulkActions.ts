...
// The photos bulk-action bar's fixed descriptor set (favorite/unfavorite, copy, download, share, trash) —
...
// as itemActions.ts's own Move exclusion; Copy stays, as it does there. Unshare/restoreSelected/
// delete/disableLink never apply either: a photos item is never a shared-root arm, never trashed
// (trash removes it from the listing), and links has no photos analogue.
export function photosBulkActions(flags: DriveSelectionFlags): BulkActionDescriptor[] {
	return [
		...
		{ id: "copy", ...ACTION_DEFS.copy, run: "dialog", dialogKind: "copy" },
